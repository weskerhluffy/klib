...
void psort(void *, size_t, size_t, int (*)(const void *, const void *)) __attribute__((availability(macosx,introduced=10.6)));
...
int main() {
	...
	kh_del_caca(h, k);
	for (k = (khint_t) (0); k != ((h)->n_buckets); ++k)
		if ((!(((h)->flags[(k) >> 4] >> (((k) & 0xfU) << 1)) & 3))) {
			printf("llave %u si existe, tiene llave %u y valor %u\n", k,
					((h)->keys[k]), ((h)->vals[k]));
			((h)->vals[k]) = 1;
		}
	kh_destroy_caca(h);
